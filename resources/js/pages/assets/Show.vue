...
import { index as assetsIndex, destroy, edit, show } from '@/routes/assets';
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="space-y-1">
                ...
                <p class="text-sm text-muted-foreground">{{ asset.category }}</p>
            </div>
            <div class="flex items-center gap-2">
                <Button variant="outline" as-child>
                    <Link :href="edit(asset.id)"><Pencil class="size-4" /> Edit</Link>
                </Button>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <div class="space-y-6 lg:col-span-2">
                <div class="rounded-xl border border-sidebar-border/70 p-5 dark:border-sidebar-border">
                    <h2 class="mb-4 text-sm font-medium text-muted-foreground">Technical specifications</h2>
                    ...
                    <div v-if="asset.notes" class="mt-4 border-t border-sidebar-border/70 pt-4 dark:border-sidebar-border">
                        <dt class="text-xs text-muted-foreground">Notes</dt>
                        <dd class="mt-1 text-sm whitespace-pre-line">{{ asset.notes }}</dd>
                    </div>
                </div>
            </div>

            <div class="rounded-xl border border-sidebar-border/70 p-5 dark:border-sidebar-border">
                <h2 class="mb-4 text-sm font-medium text-muted-foreground">Activity timeline</h2>

                <Deferred data="logs">
                    <template #fallback>
                        <div class="space-y-4">
                            <Skeleton v-for="n in 4" :key="n" class="h-12 w-full rounded-md" />
                        </div>
                    </template>

                    <ol v-if="logs && logs.length" class="relative space-y-6 border-l border-sidebar-border/70 pl-5 dark:border-sidebar-border">
                        <li v-for="log in logs" :key="log.id" class="relative">
                            <span class="absolute -left-[23px] top-1 size-3 rounded-full bg-primary ring-4 ring-background" />
                            <div class="flex items-center gap-2">
                                <Badge variant="outline">{{ log.action_label }}</Badge>
                                <span class="text-xs text-muted-foreground">{{ formatDate(log.created_at) }}</span>
                            </div>
                            <p class="mt-1 text-sm">{{ log.description }}</p>
                            <p v-if="log.user" class="text-xs text-muted-foreground">by {{ log.user }}</p>
                        </li>
                    </ol>
                    <p v-else class="text-sm text-muted-foreground">No activity recorded yet.</p>
                </Deferred>
            </div>
        </div>
    </div>
